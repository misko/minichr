...
function covNcluster {
	...
	#$j -jar $p/CollectInsertSizeMetrics.jar I=$wd/${ty}.bam H=$wd/${ty}_histo O=$wd/${ty}_stats
...
python2.6 $g/scripts/overlap.py 1000 ${wd}/normal_cov0.txt_f.gz ${wd}/tumor_cov5.txt_f.gz  |  awk '{if (NF>4) {print $0}}' | awk '{print $1"\t"$2"\t"$2+1"\t"$3"\t"$4"\t"$5"\t"$6"\t"$7}'  | awk '{print $5"\t"$6"\t"$7"\t"$1"\t"$2"\t"$4"\t"$8}' | awk '{d=$2-$5; if (d<0) {d=-d}; if ($3==$6 && $1==$4 && d<2000) { } else {print $0}}'  > $wd/nsubtract_centrosubtract_1000bp
...
echo running walker
$g/walker/walker $wd/nsubtract_centrosubtract_1000bp_joined_links $wd/nsubtract_centrosubtract_1000bp_joined_arc_coveage ${wd}/hmm_out 2 > ${wd}/walker_out
